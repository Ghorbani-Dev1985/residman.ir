...
type SortProp = {
    setSortByDate : boolean
}
const SortPostLink: React.FC<SortProp> = ({setSortByDate}) => {
    return(
        <div className="w-full flex justify-between items-center my-6">
        <p onClick={() => setSortByDate((prev : boolean) => !prev)} className="flex justify-center items-center gap-x-1.5 cursor-pointer text-zinc-400 hover:text-zinc-800 transition-colors select-none"><ArrowsUpDownIcon className="size-5"/>مرتب سازی تاریخ</p>
